package day0320;

import java.util.Date;

/*
 * 1. Person 추상클래스를 상속받는다.
 * 2. 소속학과, 학년, 재학여부를 저장하는 필드를 정의한다.
 * 3. 기본 생성자를 정의한다.
 * 4. 번호, 이름, 소속학과, 학년 , 재학여부, 생성일을 전달받아서 초기화하는 생성자를 정의한다.
 * 		(번호, 이름, 생성일의 초기화는 부모의 생성자를 활용한다.)
 * 5. 소속학과, 학년, 재학여부 필드의 getter/setter 메소드를 정의한다.
 * 6. Person으로부터 물려받은 추상메소드(String getRole())를 구현(재정의)한다.
 * 		(학생클래스는 "S" 값을 반환한다.)
 */
public class Student extends Person{

	private String dept;
	private int grade;
	private boolean enroll;
	
	public Student() {
		
	}

	public Student(int no, String name, String dept, int grade, boolean enroll, Date createDate) {
		super(no, name, createDate);
		this.dept = dept;
		this.grade = grade;
		this.enroll = enroll;
	}
	
	public String getDept() {
		return dept;
	}

	public void setDept(String dept) {
		this.dept = dept;
	}

	public int getGrade() {
		return grade;
	}

	public void setGrade(int grade) {
		this.grade = grade;
	}

	public String getEnroll() {
		if(!enroll) {
			return "휴학중";
		} else {
			return "재학중";
		}
	}

	public void setEnroll(boolean enroll) {
		this.enroll = enroll;
	}

	public String getRole() {
		return "S";
	}
	
	public void displayInfo() {
		System.out.println("번    호: " + getNo());
		System.out.println("이    름: " + getName());
		System.out.println("학    과: " + dept);
		System.out.println("학    년: " + grade);
		System.out.println("재학여부: " + getEnroll());
		System.out.println("생 성 일: " + CommonUtils.dateToString(getCreateDate()));
	}
	
}
